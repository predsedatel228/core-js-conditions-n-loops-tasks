...
function getSpiralMatrix(size) {
  ...
  while (rowStart <= rowEnd && colStart <= rowEnd) {
    ...
    for (let t = rowStart; t <= rowEnd; t += 1) {
      counter += 1;
      matrix[t][colEnd] = counter;
    }
    colEnd -= 1;

    for (let s = colEnd; s >= colStart; s -= 1) {
      counter += 1;
      matrix[rowEnd][s] = counter;
    }
    rowEnd -= 1;

    for (let n = rowEnd; n >= rowStart; n -= 1) {
      counter += 1;
      matrix[n][colStart] = counter;
    }
    colStart += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    let row = [];
    for (let t = matrix.length - 1; t >= 0; t -= 1) {
      row = [...row, matrix[t][i]];
    }
    arr = [...arr, [...row]];
  }
  for (let i = 0; i < matrix.length; i += 1) {
    for (let t = 0; t < matrix[i].length; t += 1) {
      copyMatrix[i][t] = arr[i][t];
    }
  }
  return matrix;
}
...
function shuffleChar(str, iterations) {
  ...
  let itr = iterations;
  while (itr) {
    for (let t = 0; t < copyStr.length; t += 1) {
      if (t % 2 === 0) {
        left += copyStr[t];
      } else right += copyStr[t];
    }
